"""M7.1 Data prep: IoT -> daily series + policy table (robust, flake8)."""
...
def _normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if has_metric:
        piv = (
            df[[ts_col, cols["metric"], cols["value"]] + ([asset_col] if asset_col else [])]
            .rename(
                columns={
                    ts_col: "timestamp",
                    cols["metric"]: "metric",
        ...
        piv["timestamp"] = pd.to_datetime(piv["timestamp"], errors="coerce", utc=True)
        w = piv.pivot_table(
    ...
    return out[["timestamp", "asset_id", "energy_kwh", "temp_c"]]
    ...
    try:
        ...
        for cand in ["asset_id", "id", "asset"]:
            if cand in df.columns:
                vals = df[cand].astype(str).dropna().unique().tolist()
                return vals
    except Exception:
        return []
    return []
...
def _apply_analysis_window(daily: pd.DataFrame, defaults: Dict) -> pd.DataFrame:
    wnd = defaults.get("analysis_window") or {}
    if not wnd:
        return daily
    ds = pd.to_datetime(wnd.get("before_start"))
    de = pd.to_datetime(wnd.get("before_end"))
    as_ = pd.to_datetime(wnd.get("after_start"))
    ae = pd.to_datetime(wnd.get("after_end"))
    mask = pd.Series(False, index=daily.index)
    if pd.notna(ds) and pd.notna(de):
        mask |= daily["date"].between(ds, de)
    if pd.notna(as_) and pd.notna(ae):
        mask |= daily["date"].between(as_, ae)
    if mask.any():
...
def run_m7_1(config_path: str, iot_path: str) -> Dict:
    ...
    # Optional clip to analysis window(s)
    daily = _apply_analysis_window(daily, defaults)

    # Prefer assets from daily series (actual modeling set), fallback to KG
